...
def _hujjatlar_guruh_boyicha_jamlangan(db, boshlanish, tugash=None, mahsulot_id=None):
    """/statistika/grafik-detal/{kunlik,haftalik,oylik,mavsum} va
    /telegram/kunlik uchun umumiy hisoblash - _mahsulot_bosim_statistikasi()
    va _firma_haydovchi_statistikasi()dagi BILAN BIR XIL mantiq.

    ESLATMA: avval bu 5 endpointda ham to'g'ridan-to'g'ri
    func.sum(Olchov.netto)/func.sum(Olchov.konditsion) ishlatilgan edi -
    xuddi yuqoridagi ikkitasidagi eski xatoga o'xshab bu ham XATO edi:
    bitta arava uchun bir nechta Olchov qatori bo'lsa, SQL SUM ularni
    ikki marta qo'shib, tonnajni ikki baravar hisoblardi. Bu funksiya
    berilgan davr (va ixtiyoriy mahsulot_id) uchun TUGALLANDI hujjatlarni
    oladi, HAR BIR HUJJAT uchun _olchovlar_jamlangan() orqali to'g'ri
    jamlangan netto/konditsion hisoblaydi (N+1 bo'lmasligi uchun Olchov
    qatorlari bitta so'rovda olinadi) va (Hujjat, jami_netto,
    jami_konditsion) uchligi ro'yxatini qaytaradi - chaqiruvchi o'zi
    kerakli guruhlash kalitini (soat/kun/oy/mahsulot_id) Hujjat
    obyektidan chiqarib, guruhlab yig'adi."""
    query = db.query(Hujjat).filter(
        Hujjat.holat == HujjatHolati.TUGALLANDI,
        Hujjat.created_at >= boshlanish,
    )
    if tugash is not None:
        query = query.filter(Hujjat.created_at < tugash)
    if mahsulot_id is not None:
        query = query.filter(Hujjat.mahsulot_id == mahsulot_id)
    hujjatlar = query.all()
    if not hujjatlar:
        return []
    ...
    for h in hujjatlar:
        ...
        if not jami_netto or jami_netto <= 0:
            continue
        natija.append((h, jami_netto, jami_konditsion or 0.0))
    return natija
...
# Operator tuzatish so'rovi qoldira oladigan maydonlar. namlik/ifloslik
# Hujjatda emas - so'rov TASDIQLANGANDA shu hujjatning barcha Olchov
# qatorlariga yoziladi (konditsion qayta hisoblanadi). terim_turi ATAYLAB
# yo'q (u faqat admin-boshqaruvida).
TUZATISH_SOROVI_MAYDONLARI = {
    "firma", "shofyor", "tiket_raqam", "tuda_raqam", "klass", "sinf",
    "namlik", "ifloslik", "seleksiya_navi", "qabul_qildi", "yuk_olindi",
    "dostaverka", "dostaverka_vaqt",
}

# Shu maydonlar Navbat jadvalida ham bor - tasdiqlanganda unga ham
# yoziladi (operator ekrani /navbat va /navbat/tugallanganlar dan
# o'qiydi, Hujjat'dan emas).
TUZATISH_NAVBAT_MAYDONLARI = {
    "firma", "shofyor", "tiket_raqam", "tuda_raqam",
    "klass", "sinf", "seleksiya_navi", "namlik", "ifloslik",
}

TUZATISH_SOROVI_HOLATLARI = ("kutilmoqda", "tasdiqlandi", "rad_etildi")


def _tuzatish_sorovi_dict(s: TuzatishSorovi, hujjat_raqam: str = None):
    natija = {
        "id": s.id,
        "hujjat_id": s.hujjat_id,
        "operator_login": s.operator_login,
        "maydon_nomi": s.maydon_nomi,
        "eski_qiymat": s.eski_qiymat,
        "yangi_qiymat": s.yangi_qiymat,
        "sabab": s.sabab,
        "holat": s.holat,
        "yaratilgan_vaqt": str(s.yaratilgan_vaqt) if s.yaratilgan_vaqt else None,
        "hal_qilingan_vaqt": str(s.hal_qilingan_vaqt) if s.hal_qilingan_vaqt else None,
...
@app.post("/tuzatish_sorovi")
def tuzatish_sorovi_yaratish(data: dict, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    """Operator hujjat maydonida xato ko'rsa - to'g'ridan-to'g'ri
    o'zgartirmasdan shu yerga tuzatish so'rovi qoldiradi. operator_login
    ATAYLAB tanadan emas, JWT tokendan olinadi."""
    hujjat_id = data.get("hujjat_id")
    maydon = (data.get("maydon_nomi") or "").strip()
    sabab = (data.get("sabab") or "").strip()
    yangi_qiymat = data.get("yangi_qiymat")
    eski_qiymat = data.get("eski_qiymat")

    if not hujjat_id:
        raise HTTPException(status_code=400, detail="hujjat_id ko'rsatilishi shart!")
    if maydon not in TUZATISH_SOROVI_MAYDONLARI:
        raise HTTPException(
            status_code=400,
            detail=f"'{maydon}' maydoni uchun tuzatish so'rovi yuborib bo'lmaydi!"
        )
    if not sabab:
        raise HTTPException(status_code=400, detail="Tuzatish sababi ko'rsatilishi shart!")

    hujjat = db.query(Hujjat).filter(Hujjat.id == hujjat_id).first()
    if not hujjat:
        raise HTTPException(status_code=404, detail="Hujjat topilmadi!")

    sorov = TuzatishSorovi(
        hujjat_id=hujjat_id,
        operator_login=current_user.get("sub"),
        maydon_nomi=maydon,
        eski_qiymat=None if eski_qiymat is None else str(eski_qiymat),
        yangi_qiymat=None if yangi_qiymat is None else str(yangi_qiymat),
        sabab=sabab,
        holat="kutilmoqda",
    )
    db.add(sorov)
    db.commit()
    db.refresh(sorov)
    return _tuzatish_sorovi_dict(sorov, hujjat.raqam)


@app.get("/tuzatish_sorovlar")
def tuzatish_sorovlar_royxati(holat: str = None, limit: int = 500, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    """Admin (yoki hisobchi) barcha tuzatish so'rovlarini ko'radi.
    Ixtiyoriy `holat` filtri: kutilmoqda / tasdiqlandi / rad_etildi."""
    if current_user.get("role") not in ("admin", "hisobchi"):
        raise HTTPException(status_code=403, detail="Bu amal uchun sizda ruxsat yo'q!")
    if holat is not None and holat not in TUZATISH_SOROVI_HOLATLARI:
        raise HTTPException(status_code=400, detail=f"Noma'lum holat: {holat}")
    limit = min(max(limit, 1), 2000)
    q = db.query(TuzatishSorovi, Hujjat.raqam).join(
        Hujjat, TuzatishSorovi.hujjat_id == Hujjat.id
    )
    if holat is not None:
        q = q.filter(TuzatishSorovi.holat == holat)
    yozuvlar = q.order_by(TuzatishSorovi.yaratilgan_vaqt.desc()).limit(limit).all()
...
def _tuzatish_sorovini_hal_qil(sorov: TuzatishSorovi, yangi_holat: str, current_user: dict):
    if sorov.holat != "kutilmoqda":
        raise HTTPException(
            status_code=400,
            detail=f"So'rov allaqachon hal qilingan (holat: {sorov.holat})!"
        )
    sorov.holat = yangi_holat
    sorov.hal_qilingan_vaqt = datetime.now()
    sorov.admin_login = current_user.get("sub")


@app.post("/tuzatish_sorovi/{sorov_id}/tasdiq")
def tuzatish_sorovi_tasdiqlash(sorov_id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    """Admin so'rovni tasdiqlaydi - AYNAN shu yerda hujjat (va Navbat
    qatori, namlik/ifloslik uchun barcha Olchov qatorlari) haqiqatan
    yangilanadi. O'zgarish TahrirTarixi'ga ham yoziladi."""
    if current_user.get("role") not in ("admin", "hisobchi"):
        raise HTTPException(status_code=403, detail="Bu amal uchun sizda ruxsat yo'q!")
    sorov = db.query(TuzatishSorovi).filter(TuzatishSorovi.id == sorov_id).first()
    if not sorov:
        raise HTTPException(status_code=404, detail="So'rov topilmadi!")
    if sorov.maydon_nomi not in TUZATISH_SOROVI_MAYDONLARI:
        raise HTTPException(status_code=400, detail=f"'{sorov.maydon_nomi}' maydonini yangilab bo'lmaydi!")

    hujjat = db.query(Hujjat).filter(Hujjat.id == sorov.hujjat_id).first()
    if not hujjat:
        raise HTTPException(status_code=404, detail="Hujjat topilmadi!")

    maydon = sorov.maydon_nomi
    yangi_qiymat = sorov.yangi_qiymat

    if maydon in ("namlik", "ifloslik"):
        ...
        try:
            navbat_qiymat = None if yangi_qiymat in (None, "") else float(str(yangi_qiymat).replace(",", "."))
        except (TypeError, ValueError):
            raise HTTPException(status_code=400, detail=f"'{yangi_qiymat}' - {maydon} uchun yaroqli son emas!")
        eski_qiymat_matn = ", ".join(
            (str(getattr(o, maydon)) if getattr(o, maydon) is not None else "—") for o in olchovlar
        )
        for o in olchovlar:
            setattr(o, maydon, navbat_qiymat)
        for o in olchovlar:
            # namlik=0 / ifloslik=0 haqiqiy qiymat (falsy emas) - `is not None`
            if o.netto is not None and o.namlik is not None and o.ifloslik is not None:
                o.konditsion = konditsion_hisobla(o.netto, o.namlik, o.ifloslik)
        yangi_qiymat_matn = "—" if navbat_qiymat is None else str(navbat_qiymat)
    else:
        eski_val = getattr(hujjat, maydon)
        eski_qiymat_matn = None if eski_val is None else str(eski_val)
        setattr(hujjat, maydon, yangi_qiymat)
        if maydon == "firma":
            firma_royxatga_qoshish(db, yangi_qiymat)
        yangi_qiymat_matn = None if yangi_qiymat is None else str(yangi_qiymat)
        navbat_qiymat = yangi_qiymat

    if maydon in TUZATISH_NAVBAT_MAYDONLARI:
        from models import Navbat
        navbat = db.query(Navbat).filter(Navbat.hujjat_id == hujjat.id).first()
        if navbat is not None:
            setattr(navbat, maydon, navbat_qiymat)

    db.add(TahrirTarixi(
        hujjat_id=hujjat.id,
        maydon=maydon,
        eski_qiymat=eski_qiymat_matn,
        yangi_qiymat=yangi_qiymat_matn,
        sabab=f"[Tuzatish so'rovi #{sorov.id}, operator: {sorov.operator_login}] {sorov.sabab}",
        ozgartirgan_user_id=current_user.get("id"),
        ozgartirgan_username=current_user.get("sub"),
    ))

    _tuzatish_sorovini_hal_qil(sorov, "tasdiqlandi", current_user)
    db.commit()
    db.refresh(sorov)
    return _tuzatish_sorovi_dict(sorov, hujjat.raqam)


@app.post("/tuzatish_sorovi/{sorov_id}/rad")
def tuzatish_sorovi_rad_etish(sorov_id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    """Admin so'rovni rad etadi - hujjat O'ZGARMAYDI, faqat so'rov holati
    "rad_etildi" ga o'tadi."""
    if current_user.get("role") not in ("admin", "hisobchi"):
        raise HTTPException(status_code=403, detail="Bu amal uchun sizda ruxsat yo'q!")
    sorov = db.query(TuzatishSorovi).filter(TuzatishSorovi.id == sorov_id).first()
    if not sorov:
...
def telegram_kunlik(db: Session = Depends(get_db), current_user: dict = Depends(require_role("admin"))):
    ...
    def _mahsulot_boyicha_guruhlash(natijalar):
        guruhlar = {}
        for h, jami_netto, jami_konditsion in natijalar:
            g = guruhlar.setdefault(h.mahsulot_id, [0, 0.0, 0.0])
            g[0] += 1
            g[1] += jami_netto
            g[2] += jami_konditsion
    ...
    yb = _mahsulot_boyicha_guruhlash(
        _hujjatlar_guruh_boyicha_jamlangan(db, bugun))
    chigit_son, chigit_netto, chigit_kond = yb.get(1, bosh3)
    chiganoq_son, chiganoq_netto, _ = yb.get(2, bosh3)
    pochog_son, pochog_netto, _ = yb.get(3, bosh3)
    patoz_son, patoz_netto, _ = yb.get(4, bosh3)

    ym = _mahsulot_boyicha_guruhlash(
        _hujjatlar_guruh_boyicha_jamlangan(db, mavsum_boshi))
    mchigit_son, mchigit_netto, mchigit_kond = ym.get(1, bosh3)
    mchiganoq_son, mchiganoq_netto, _ = ym.get(2, bosh3)
    mpochog_son, mpochog_netto, _ = ym.get(3, bosh3)
    mpatoz_son, mpatoz_netto, _ = ym.get(4, bosh3)
...
def grafik_detal_kunlik(mahsulot: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    soat_dict = {}
    for h, jami_netto, _ in _hujjatlar_guruh_boyicha_jamlangan(
            db, bugun, ertaga, mahsulot_id=mahsulot_id):
        soat = h.created_at.hour
        soni, netto = soat_dict.get(soat, (0, 0.0))
        soat_dict[soat] = (soni + 1, netto + jami_netto)
...
def grafik_detal_haftalik(mahsulot: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    kun_dict = {}
    for h, jami_netto, _ in _hujjatlar_guruh_boyicha_jamlangan(
            db, hafta_boshi, keyingi_hafta, mahsulot_id=mahsulot_id):
        kun = h.created_at.isoweekday()
        soni, netto = kun_dict.get(kun, (0, 0.0))
        kun_dict[kun] = (soni + 1, netto + jami_netto)
...
def grafik_detal_oylik(mahsulot: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    kun_dict = {}
    for h, jami_netto, _ in _hujjatlar_guruh_boyicha_jamlangan(
            db, oy_boshi, keyingi_oy, mahsulot_id=mahsulot_id):
        kun = h.created_at.day
        soni, netto = kun_dict.get(kun, (0, 0.0))
        kun_dict[kun] = (soni + 1, netto + jami_netto)
...
def grafik_detal_mavsum(mahsulot: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    oy_dict = {}
    for h, jami_netto, _ in _hujjatlar_guruh_boyicha_jamlangan(
            db, mavsum_boshi, mavsum_oxiri, mahsulot_id=mahsulot_id):
        kalit = (h.created_at.year, h.created_at.month)
        soni, netto = oy_dict.get(kalit, (0, 0.0))
        oy_dict[kalit] = (soni + 1, netto + jami_netto)

...
# Operator tuzatish so'rovi qoldira oladigan maydonlar. DIQQAT: terim_turi
# ATAYLAB yo'q. namlik/ifloslik Hujjatda emas - ular so'rov TASDIQLANGANDA
# shu hujjatning barcha Olchov qatorlariga yoziladi (konditsion qayta
# hisoblanadi), qolgan hammasi to'g'ridan-to'g'ri Hujjat ustuni.
TUZATISH_SOROVI_MAYDONLARI = {
    "firma", "klass", "sinf", "namlik", "ifloslik", "tiket_raqam",
    "tuda_raqam", "seleksiya_navi", "qabul_qildi", "yuk_olindi",
    "dostaverka", "dostaverka_vaqt",
}
...
def tuzatish_sorovi_yaratish(data: dict, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    """Operator "Tugallanganlar" ro'yxatidagi hujjat maydonida xato ko'rsa,
    to'g'ridan-to'g'ri o'zgartirmasdan shu yerga tuzatish so'rovi qoldiradi.
    operator_login ATAYLAB tanadan emas, JWT tokendan olinadi."""
...
def tuzatish_sorovlar_royxati(holat: str = None, limit: int = 200, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
...
def tuzatish_sorovlar_operator(login: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    """Operator o'z tuzatish so'rovlarini (va ularning holatini) ko'radi.
    Operator faqat O'Z loginini so'rashi mumkin; admin/hisobchi - istalganini."""
...
def _tuzatish_sorovini_hal_qil(sorov: TuzatishSorovi, yangi_holat: str, current_user: dict, db: Session):
...
def tuzatish_sorovi_tasdiqlash(sorov_id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    """Admin so'rovni tasdiqlaydi - AYNAN shu yerda hujjat haqiqatan
    yangilanadi. namlik/ifloslik hujjatning barcha aravalariga qo'llanadi
    va konditsion qayta hisoblanadi. O'zgarish TahrirTarixi'ga ham yoziladi
    (audit izchilligi uchun)."""
    if current_user.get("role") not in ("admin", "hisobchi"):
        raise HTTPException(status_code=403, detail="Bu amal uchun sizda ruxsat yo'q!")
    ...
    if maydon in ("namlik", "ifloslik"):
        ...
        try:
            son = None if yangi_qiymat in (None, "") else float(str(yangi_qiymat).replace(",", "."))
        ...
        for o in olchovlar:
            setattr(o, maydon, son)
        for o in olchovlar:
            # namlik=0 / ifloslik=0 haqiqiy qiymat (falsy emas) - `is not None`
            if o.netto is not None and o.namlik is not None and o.ifloslik is not None:
                o.konditsion = konditsion_hisobla(o.netto, o.namlik, o.ifloslik)
        eski_qiymat_matn = eski_matn
        yangi_qiymat_matn = "—" if son is None else str(son)
    else:
    ...
    _tuzatish_sorovini_hal_qil(sorov, "tasdiqlandi", current_user, db)
    db.commit()
    db.refresh(sorov)
    return _tuzatish_sorovi_dict(sorov, hujjat.raqam)
...
def tuzatish_sorovi_rad_etish(sorov_id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    if not sorov:
        raise HTTPException(status_code=404, detail="So'rov topilmadi!")
    _tuzatish_sorovini_hal_qil(sorov, "rad_etildi", current_user, db)
    db.commit()
    db.refresh(sorov)

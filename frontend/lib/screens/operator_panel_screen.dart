...
class _OperatorPanelScreenState extends State<OperatorPanelScreen>
  ...
  // Shu operator "kutilmoqda" holatidagi tuzatish so'rovi yuborgan
  // hujjatlarning id'lari - navbat/tugallanganlar ro'yxatida yashil
  // ajralib turadi. Har 3 soniyada backend'dan yangilanadi (_tuzatishTimer).
  final Set<int> _tuzatishSoroviYuborilgan = {};
  // So'nggi ko'rilgan holatlar (id -> holat) - "kutilmoqda"dan boshqa
  // holatga o'tishni aniqlab, navbat ma'lumotini darhol yangilash uchun.
  final Map<int, String> _tuzatishSorovHolati = {};
  Timer? _tuzatishTimer;
 ...
 List<NavbatMashina> get navbat =>
  ...
  void initState() {
    ...
    navbatYangilashTimer = Timer.periodic(
        ...
        (_) => _backendDanYukla());
    // Tuzatish so'rovlari alohida, TEZROQ (3s) siklda - admin tasdiqlasa
    // operator 3 soniya ichida ko'rishi uchun.
    _tuzatishTimer = Timer.periodic(
        const Duration(seconds: 3),
        (_) => _tuzatishSorovlariniYukla());
  ...
  Future<void> _tuzatishSorovlariniYukla() async {
    final sorovlar =
        await ApiService.tuzatishSorovlarOperator(widget.username);
    if (!mounted) return;
    final kutilayotgan = <int>{};
    var halQilingan = false;
    for (final s in sorovlar) {
      final id = (s['hujjat_id'] ?? 0) as int;
      final holat = (s['holat'] ?? '') as String;
      if (holat == 'kutilmoqda') kutilayotgan.add(id);
      final sorovId = (s['id'] ?? 0) as int;
      final oldingi = _tuzatishSorovHolati[sorovId];
      if (oldingi == 'kutilmoqda' && holat != 'kutilmoqda') {
        halQilingan = true;
      }
      _tuzatishSorovHolati[sorovId] = holat;
    }
    final farqli = kutilayotgan.length !=
            _tuzatishSoroviYuborilgan.length ||
        !_tuzatishSoroviYuborilgan.containsAll(kutilayotgan);
    if (farqli) {
      setState(() {
        _tuzatishSoroviYuborilgan
          ..clear()
          ..addAll(kutilayotgan);
      });
    }
    // Biror so'rov endigina tasdiqlangan/rad etilgan bo'lsa - navbat
    // ma'lumotini DARHOL yangilaymiz (admin o'zgartirishi ko'rinsin).
    if (halQilingan) _backendDanYukla();
  }
  ...
  // ko'rinadigan nom, raqammi). terim_turi ATAYLAB yo'q.
  static const List<List<dynamic>> _tuzatishMaydonlari = [
    ['firma', 'Firma', false],
    ['shofyor', 'Shofyor', false],
    ['tiket_raqam', 'Tiket №', false],
    ['tuda_raqam', 'Tuda №', false],
    ['klass', 'Klass', false],
    ['sinf', 'Sinf', false],
    ['namlik', 'Namlik %', true],
    ['ifloslik', 'Ifloslik %', true],
    ['seleksiya_navi', 'Seleksiya navi', false],
    ['qabul_qildi', 'Qabul qildi', false],
    ['yuk_olindi', 'Yuk olindi', false],
    ['dostaverka', 'Dostaverka №', false],
    ['dostaverka_vaqt', 'Dostaverka muddati', false],
  ];

  void mashinaKor(NavbatMashina mashina) async {
    final offline =
        OfflineQueueService.yerliIdmi(mashina.hujjatId);
    // dostaverka/dostaverka_vaqt/qabul_qildi/yuk_olindi NavbatMashina'da
    // yo'q - to'liq ma'lumotni backend'dan olamiz.
    Map<String, dynamic>? toliq;
    if (!offline) {
      toliq = await ApiService.getHujjat(mashina.hujjatId);
    }
    if (!mounted) return;

    String aslQiymat(String kalit) {
      final v = toliq?[kalit];
      ...
      switch (kalit) {
        case 'firma':
          return mashina.firma.trim();
        case 'shofyor':
          return mashina.shofyor.trim();
        case 'tiket_raqam':
          return mashina.tiketRaqam?.trim() ?? '';
        case 'tuda_raqam':
          return mashina.tudaRaqam?.trim() ?? '';
        case 'klass':
          return mashina.klass?.trim() ?? '';
        case 'sinf':
          return mashina.sinf?.trim() ?? '';
        case 'namlik':
          return mashina.namlik?.toString() ?? '';
        case 'ifloslik':
          return mashina.ifloslik?.toString() ?? '';
        case 'seleksiya_navi':
          return mashina.seleksiyaNavi?.trim() ?? '';
        case 'qabul_qildi':
          return mashina.qabulQildi?.trim() ?? '';
        case 'yuk_olindi':
          return mashina.yukOlindi?.trim() ?? '';
        default:
          return '';
      }
    }
    ...
    final ctrls = <String, TextEditingController>{
      for (final m in _tuzatishMaydonlari)
        m[0] as String:
            TextEditingController(text: aslQiymat(m[0] as String)),
    };
    final asl = {
      for (final e in ctrls.entries) e.key: e.value.text.trim()
    };
    final sababCtrl = TextEditingController();
    bool tahrirRejim = false;
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              child: Column(
                  ...
                  children: [
                _korSection("🚛 MASHINA", [
                  _korRow("Davlat raqami", mashina.raqam),
                  ...
                  if (mashina.bekorSababi != null)
                    _korRow("Bekor sababi", mashina.bekorSababi!),
                ]),
                const SizedBox(height: 10),
                _korSection(
                    tahrirRejim
                        ? "📋 HUJJAT — TUZATISH SO'ROVI"
                        : "📋 HUJJAT MA'LUMOTLARI",
                    [
                      for (final m in _tuzatishMaydonlari)
                        tahrirRejim
                            ? _tuzatishTahrirField(
                                m[1] as String,
                                ctrls[m[0]]!,
                                m[2] as bool)
                            : _korRow(
                                m[1] as String,
                                ctrls[m[0]]!.text.isEmpty
                                    ? '—'
                                    : ctrls[m[0]]!.text),
                    ]),
                const SizedBox(height: 10),
                _korSection("⚖️ O'LCHOVLAR", [
                  for (int i = 1; i <= 3; i++)
                    if (mashina.aravalar[i]?.tara != null)
                      _olchovRow(i, mashina.aravalar[i]!),
                  if (!mashina.aravalar.values
                      .any((a) => a.tara != null))
                    _korRow("Ma'lumot", "Hali o'lchanmagan"),
                ]),
                if (tahrirRejim) ...[
                  const SizedBox(height: 10),
                  TextField(
                  ...
                  const Text(
                      "So'rov adminga yuboriladi. Admin tasdiqlagach ma'lumot yangilanadi.",
                      style: TextStyle(
          ...
          actions: [
            if (!tahrirRejim) ...[
              ...
              if (!offline)
                ElevatedButton.icon(
                  onPressed: () =>
                      setDlg(() => tahrirRejim = true),
                  icon: const Icon(Icons.edit_note, size: 16),
                  label: const Text(
                      "Tuzatish so'rovi yuborish",
                      style: TextStyle(fontSize: 12)),
                  style: ElevatedButton.styleFrom(
                      backgroundColor: brandGreen,
                      foregroundColor: Colors.white,
              ...
              TextButton(
                  onPressed: yuborilmoqda
                      ? null
                      : () => setDlg(() => tahrirRejim = false),
                  child: const Text("Orqaga")),
              ElevatedButton.icon(
                onPressed: yuborilmoqda
                    ? null
                    : () async {
                        final sabab = sababCtrl.text.trim();
                        final ozgargan = ctrls.entries
                            .where((e) =>
                                e.value.text.trim() !=
                                (asl[e.key] ?? ''))
                            .toList();
                        if (ozgargan.isEmpty) {
                          _xabar("Hech qanday maydon o'zgartirilmadi");
                          return;
                        }
                        if (sabab.isEmpty) {
                          _xabar("❌ Sabab kiritilishi SHART!");
                          return;
                        }
                        setDlg(() => yuborilmoqda = true);
                        final xatolar = <String>[];
                        for (final e in ozgargan) {
                          final eski = asl[e.key] ?? '';
                          final yangi = e.value.text.trim();
                          final xato = await ApiService
                              .tuzatishSoroviYuborish(
                            hujjatId: mashina.hujjatId,
                            maydonNomi: e.key,
                            eskiQiymat: eski.isEmpty ? null : eski,
                            yangiQiymat:
                                yangi.isEmpty ? null : yangi,
                            sabab: sabab,
                          );
                          if (xato != null) {
                            final nom = _tuzatishMaydonlari
                                .firstWhere((m) => m[0] == e.key)[1];
                            xatolar.add("$nom: $xato");
                          }
                        }
                        if (!mounted) return;
                        if (xatolar.isEmpty) {
                          setState(() => _tuzatishSoroviYuborilgan
                              .add(mashina.hujjatId));
                          Navigator.pop(ctx);
                          _xabar(
                              "✅ Tuzatish so'rovi yuborildi (${ozgargan.length} ta maydon)");
                        } else {
                          setDlg(() => yuborilmoqda = false);
                          _xabar(
                              "Ba'zi so'rovlar yuborilmadi: ${xatolar.join('; ')}");
                        }
                      },
                icon: yuborilmoqda
                    ? const SizedBox(
  ...
  Widget _tuzatishTahrirField(
      String label, TextEditingController ctrl, bool raqam) {
    return Padding(
...
try {
  ...
  Widget navbatItem(NavbatMashina mashina) {
    ...
    final sorovYuborilgan =
        _tuzatishSoroviYuborilgan.contains(mashina.hujjatId);
    return GestureDetector(
          ...
          border: Border.all(
              color: tanlangan
                  ...
                  : (sorovYuborilgan
                      ? brandGreen
                      : brandGreenBorder),
              width: (tanlangan || sorovYuborilgan) ? 2 : 1),
          borderRadius: BorderRadius.circular(10),
        ),
        child: Row(children: [
          Container(
